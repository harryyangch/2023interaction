...
void initializeMaze() {
  ...
  // 隨機生成迷宮，確保終點及出生點周圍沒有牆壁
  do {
    for (int i = 0; i < maze.length; i++) {
      maze[i] = random(1) > 0.8 ? 1 : 0;
    }
    goalIndex = maze.length - 1;
  } while (maze[goalIndex] == 1 || isPlayerStartSurrounded());
...
// 檢查出生點是否被牆壁包圍
boolean isPlayerStartSurrounded() {
  int x = playerIndex % cols;
  int y = floor(playerIndex / cols);

  // 檢查上下左右四個方向
  if ((x > 0 && maze[playerIndex - 1] == 1) ||
      (x < cols - 1 && maze[playerIndex + 1] == 1) ||
      (y > 0 && maze[playerIndex - cols] == 1) ||
      (y < rows - 1 && maze[playerIndex + cols] == 1)) {
    return true;
  }
...
void mousePressed() {
  if (!gameStarted && !gameFinished && !gameFailed && mouseX > width / 4 && mouseX < width * 3 / 4 && mouseY > height - 40 && mouseY < height - 10) {
    // 重新開始按鈕被按下
    restartGame();
  } else if (!gameStarted) {
    // 遊戲未開始時按下滑鼠開始遊戲
